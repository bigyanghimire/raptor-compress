...
#define CHECK_CUSPARSE(func)                                               \
    ...
    double *spmv_gpu(const CSRMatrix *A, const double *x)
    {
        ...
        const double *hX = x;
        // double *hB = b;
        double *hB = (double *)malloc(A_num_rows * sizeof(double));
        memset(hB, 0, A_num_rows * sizeof(double)); // manually set all bytes to 0
        ...
        CHECK_CUDA(cudaMemcpy(dX, hX, A_num_cols * sizeof(double),
                              cudaMemcpyHostToDevice));
        CHECK_CUDA(cudaMemcpy(dB, hB, A_num_rows * sizeof(double),
                              cudaMemcpyHostToDevice));
        // //-----------------------------------------------------  CUSPARSE APIs
        ...
        CHECK_CUDA(cudaMemcpy(hB, dB, A_num_rows * sizeof(double),
