...
int main(int argc, char *argv[])
{
    ...
    double *stencil = NULL;
    time_matrix_setup_start = MPI_Wtime();
    stencil = diffusion_stencil_2d(eps, theta);
    A = par_stencil_grid(stencil, grid.data(), dim);
    time_matrix_setup_end = MPI_Wtime() - time_matrix_setup_start;
    delete[] stencil;
    // SZ
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    time_base = MPI_Wtime();
    ext_solve_tol=std::stod(argv[2]);
    c_info.compression_on=std::stoi(argv[3]);
    //compression_on=1;
    int total_iter=ml->solve(x, b);
    c_info.compression_on=81;
